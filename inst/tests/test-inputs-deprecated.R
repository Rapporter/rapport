...
test_that('should have length 1', {
    expect_that(rapport:::guess.old.input.length("", "variable"), equals(list(exactly = 1)))
    expect_that(rapport:::guess.old.input.length("[]", "variable"), equals(list(exactly = 1)))
    expect_that(rapport:::guess.old.input.length("[1,]", "variable"), equals(list(exactly = 1)))
    expect_that(rapport:::guess.old.input.length("[10,]", "variable"), equals(list(exactly = 10)))
})

test_that('should have correct length', {
    expect_that(rapport:::guess.old.input.length("[1,20]", "variable"), equals(list(min = 1, max = 20)))
    expect_that(rapport:::guess.old.input.length("[20]", "variable"), equals(list(exactly = 20)))
...
test_that('should provide correct input definition', {
    ...
    expect_that(rapport:::guess.old.input.type("string"), equals(list(class = 'character', length = list(exactly = 1), value = NULL, nchar = list(min = 1, max = 256), required = FALSE, standalone = TRUE)))
    expect_that(rapport:::guess.old.input.type("fee, fi, foo, fam"), equals(list(class = 'character', length = list(exactly = 1), value = c("fee", "fi", "foo", "fam"), matchable = TRUE, required = FALSE, standalone = TRUE)))
})
