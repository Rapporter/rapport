...
<%=
if (length(var) < 5000) {" - **Shapiro-Wilk test** is a powerful normality test appropriate for small samples. In R, it's implemented in `shapiro.test` function available in `stats` package."}
%>
...
Here you can see the results of applied normality tests ([p-values](http://en.wikipedia.org/wiki/P-value) less than 0.05 indicate significant discrepancies):

<%=
if (length(var) > 5000) {
    h <- htest(var, lillie.test, ad.test)
} else {
    h <- htest(var, lillie.test, ad.test, shapiro.test)
}
p <- .05
h
-%>
...
<% } -%>
<% if (!is.na(h[3, 3])) { -%>
 - according to _Shapiro-Wilk test_, the distribution of _<%= var.label %>_ is <%= ifelse(h[3, 3] < p, "not", "") %> normal.
<% } -%>
