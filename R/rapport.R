...
#'          \item \code{\%D}: an auto-incrementing integer (increments while each \code{\link{rapport}} call),
#'          \item \code{\%d}: the index of the generating chunk (in the same \code{rapport}). This latter is important to save images with different IDs.
#'      }
#'     \item \code{rp.file.path}: a directory where generated images and exported documents would take place. Please note, that the path should not contain any spaces at all (exception of this rule is on Windows where \code{shortPathName} could handle that problem with traditional directories like "Documents and Settings" etc.).
#'     \item \code{portable.html}: a boolean indicating if HTML includes (JS, CSS, fonts, images) should be copied to given \code{rp.file.path} directory or the exported documents could use these resources from the package directory.
