...
#'
#' \itemize{
#'     \item \emph{tpl.user}: a (user)name to show in exported report (defaults to \code{"Anonymous"})
#'     \item \emph{tpl.email}: a user's e-mail address to show in exported report (defaults to an empty string).
#'     \item \emph{rapport.mode} defines the manner in which \code{\link{rapport}} evaluates the templates - by default it is set to \emph{normal}. In \emph{normal} mode \code{link{rapport}} would check all non-strict templates (see: \code{\link{tpl.check}}) "linewise", and that can cause huge overhead, while \emph{performance} mode would not perform cautious checks and evaluate all commands at once. That is not a problem if you write \emph{strict} templates, which is really advised. \emph{debug} mode is  appropriate for developers, as it will interrupt the execution if template contains any errors.
#'     \item By default \code{\link{rapport}} function saves plots to image files (see the settings below) and \code{print} method just shows the path(s) of the generated image(s). If you would like to see the plot(s) when calling \code{rapport} function from an interactive R console, please set \code{graph.record} and \code{graph.replay} options to \code{TRUE} beforehand. In that case all generated plots will be displayed after printing the \code{rapport} object. These options are set to \code{FALSE} by default although we find these settings really handy, as you can resize the images on the fly and export resized images to HTML/ODT/DOCX/PDF etc. If you would even like to save the actual environment of each generated plot (variables, data sets etc.) as an \code{RData} file, please set \code{graph.save.env} to \code{TRUE}.
#'     \item \code{\link{rapport}} also has some options to set formatting style of numbers, characters and dates. The decimals needed for automatic rounding is defined in \code{rp.decimal} and {rp.decimal.short} options which are by default set to \code{4} and \code{2}, respectively. Inline numeric values are presented in a short form, while the tables use the long form. The decimal mark can be set easily to e.g. \code{","} in \code{rp.decimal.mark} option, and the date format can be specified in POSIX format via \code{rp.date.format} option.
#'     \item the exported graphs can be customised via following options:
#'     \itemize{
#'         \item The most basic option is \code{style.theme} which points to a \code{lattice}/\code{trellis} theme, and uses default \emph{rapport} theme: \code{\link{theme.rapport}}. If you do not like this minimalistic bluish theme, you might reconsider using e.g. \code{standard.theme()} from \code{lattice}, \code{ggplot2like()} or \code{theEconomist.theme()} themes available in \code{latticeExtra} package or either \code{custom.theme.black()} from \code{latticist} package. Of course, custom theme might be provided too - check out the \code{?custom.theme} from \code{latticeExtra} package or head directly to: \code{\link{trellis.par.get()}}.
